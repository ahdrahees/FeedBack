<script lang="ts">
	...
	onMount(async () => {
		input = document.getElementById('brand') as HTMLInputElement;
	});
	...
	const post = async () => {
		...
		const result = await postBrand(brand);
...
<div class="flex justify-between mb-10">
	<label class="label flex-grow mr-4">
		<input bind:value={brand} id="brand" class="input" type="text" placeholder="Brand" />
	</label>
